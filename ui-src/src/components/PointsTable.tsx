...
const ModelTable: React.FC<ModelTableProps> = ({
  ...
  return (
    <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-sm overflow-hidden transition-all duration-300">
      {/* Model Header */}
      <div
        className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-700/30 border-b border-slate-200 dark:border-slate-700 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-700/50 transition-colors duration-200"
        onClick={onToggle}
      >
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h3 className="text-lg font-semibold text-slate-800 dark:text-white">{modelName}</h3>
            <p className="text-sm text-slate-500 dark:text-slate-400">{modelDescription}</p>
            <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">Model {modelId} • {points.length} points</p>
          </div>
        </div>
        <div className="flex items-center space-x-2">
          <span className="text-xs bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 px-3 py-1 rounded-full font-medium">
              ...
              <tr>
                <th className="px-6 py-3 text-left">
                  <button
                    ...
                    className="flex items-center space-x-2 text-sm font-semibold text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white transition-colors duration-200"
                  >
                ...
                <th className="px-6 py-3 text-center">
                  <span className="text-sm font-semibold text-slate-700 dark:text-slate-300">Actions</span>
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
              {sortedPoints.map((item, index) => (
                <tr
                  ...
                  className="hover:bg-slate-50 dark:hover:bg-slate-700/20 transition-colors duration-200"
                >
                  ...
                  <td className="px-6 py-3">
                    <div className="flex flex-wrap gap-1">
                      {item.units.slice(0, 3).map(unit => (
                        <span
                          key={unit}
                          className="inline-flex items-center px-2 py-1 rounded-full text-xs bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400"
                        >
                          {unit}
                        </span>
                      ))}
                      {item.units.length > 3 && (
                        <span className="inline-flex items-center px-2 py-1 rounded-full text-xs bg-slate-100 dark:bg-slate-700/50 text-slate-600 dark:text-slate-400">
                  ...
                  <td className="px-6 py-3 text-center">
                    <button
                      ...
                      className="inline-flex items-center space-x-1 px-3 py-1.5 text-sm bg-blue-600 dark:bg-blue-600 text-white rounded-lg hover:bg-blue-700 dark:hover:bg-blue-500 transition-colors duration-200 shadow-sm"
                    >
...
export const PointsTable: React.FC<PointsTableProps> = ({ points, onPointClick, data }) => {
  ...
  if (categorizedPoints.size === 0) {
    return (
      <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-sm p-12 text-center transition-all duration-300">
        <BarChart3 className="w-12 h-12 text-slate-300 dark:text-slate-600 mx-auto mb-4" />
        <h3 className="text-lg font-medium text-slate-500 dark:text-slate-400 mb-2">No points found</h3>
